...
def purchase():
    ...
    if not product_name or not price_str or not quantity_str:
    ...
    if product:
        if manager.saldo >= price * quantity:
            product.quantity += quantity
            transaction = Transaction(operation="Zakup", product_id=product.id, quantity=quantity)
            db.session.add(product)
            db.session.add(transaction)
            manager.saldo -= price * quantity  # Zmniejsz saldo o koszt zakupu
            db.session.commit()
        else:
            flash("Nie masz wystarczająco środków na koncie", 'error')
    else:
        product = Product(name=product_name, quantity=quantity, price=price)
        transaction = Transaction(operation="Zakup", product=product, quantity=quantity)
        if manager.saldo >= price * quantity:
...
def sale():
    ...
    if product:
        if product.quantity >= quantity:
            ...
            transaction = Transaction(operation="Sprzedaż", product_id=product.id, quantity=quantity)
            db.session.add(transaction)
            manager.saldo += product.price * quantity  # Zwiększ saldo o wartość sprzedaży
            db.session.commit()
        else:
            flash(f"Nie udało się sprzedać towaru {product_name}, zbyt mała ilość na magazynie", 'error')
            db.session.rollback()
    else:
        flash(f"Produkt {product_name} nie istnieje", 'error')
